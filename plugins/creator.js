let handler = async(m, { conn }) => {
  ...
  const sentMsg = await conn.sendContactArray(m.chat, [
    [`${owner[0]}`, `${await conn.getName(owner[0]+'@s.whatsapp.net')}`, `👑 Developer Bot `, `🚫 Don't call me 🥺`, `kevinaafsar@gmail.com`, `🇮🇩 Indonesia`, `🚀 https://kevinafsarmanik.github.io/bucinadeep/`, `👤 Gada pawang nih senggol dong 😔`],
    [`${conn.user.jid.split('@')[0]}`, `${await conn.getName(conn.user.jid)}`, `🔥 Bot WhatsApp 🐣`, `📵 Don't spam/call me 😢`, `Nothing`, `🇮🇩 Indonesia`, `🚀 https://kevinafsarmanik.github.io/nino-wife.github.io/`, `🤖 Hanya bot biasa yang kadang suka eror ☺`]
  ], fkonn) 
